...
def crear_pais(request):
    ...
    try:
        profiles = Profile.objects.get(user=usuario)
    except Profile.DoesNotExist:
        profiles = None
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect(reverse('cms:lista_paises'))
    else:
        form = PaisForm()
...
def crear_region(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect(reverse('cms:lista_region'))
    else:
        form = RegionForm()
...
def crear_comuna(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect(reverse('cms:lista_comuna'))
    else:
        form = ComunaForm()
...
def lista_comuna(request):
    # Alias para compatibilidad con urls que esperan 'lista_comuna'
    return lista_comunas(request)
...
def crear_empresa(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect(reverse('cms:lista_empresa'))
    else:
        form = EmpresaForm()
...
def crear_representante(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect(reverse('cms:lista_representante'))
    else:
...
def crear_colegio(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect(reverse('cms:lista_colegio'))
    else:
        form = ColegioForm()
...
def graba_perfil(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect(reverse('cms:perfil', kwargs={'username': request.user.username}))
    else:
...
def mostrar_perfil(request, username):
    ...
    try:
        profiles = Profile.objects.get(user=usuario)
        regiones = profiles.region
        comunas = profiles.comuna
        paises = profiles.pais
    except Profile.DoesNotExist:
        profiles = None
        regiones = None
        comunas = None
